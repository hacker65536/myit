...
ビジネスニーズに迅速に対応できるようにアカウントを設計・管理することは、それに伴う人員数の直線的な増加を必要としないようにするために重要です。少数のアカウント管理から脱却することを検討する場合、効率的にそれを実現するためのプロセスや自動化を確立する作業を考慮する必要がある。

例えば、新しいビジネスイニシアチブのために新しいアカウントを作成するようなアカウント設計を実施する場合、標準構成に基づく環境を迅速かつ確実にプロビジョニングできるよう、自動化を行うことが有益である。また、自動化により、コンプライアンスを監視し、時間の経過とともに基本構成の更新を適用することができます。

## Use Multi-factor authentication

多要素認証（MFA）は、権限レベルやアクセスメカニズムに関係なく、ルートおよびアカウント内のすべてのAWSユーザーで使用する必要があります。現在推奨されているAWSアカウント（管理アカウントまたはメンバーアカウント）のMFAベストプラクティスに従って、AWS環境全体にMFAを設定することができます。

# Break glass access

組織管理アカウントは、組織内のAWSアカウントにブレイクグラスアクセスを提供するために使用される。ブレイクグラス（火災報知器のガラスを割ることからその名がついた）とは、特定のAWSアカウントへのアクセス権限を持たない人が、承認されたプロセスを用いて例外的にアクセスを取得するための迅速な手段を指す。

組織内のAWSアカウントへの通常のアクセスは、フェデレートアクセスを使用して提供されるべきである。IAM ユーザーの使用と作成は、ブレークグラスユーザーを除き、非常に推奨されない。一般的に、組織は少なくとも2人のIAMユーザーを作成し、そのうちの1人が利用できない場合のロックダウンを防止し、組織の規模に基づいて最大4つのIAMユーザーを作成することが推奨されます。これらの役割は、管理アカウントの自動化ツールがガードレールを適用および更新し、自動化ツールの問題をトラブルシューティングおよび解決し、次の段落で詳述する承認された例外シナリオの下でガードレールをバイパスすることを許可するために必要である。

これらのユーザには、ハードウェアベースのMFAデバイスを構成することを推奨する。このデバイスは、例外的な状況において、役割を引き受けることによって、組織の管理アカウントまたは組織内のサブアカウントへのアクセスを得るために使用されることになる。ブレイクグラスアクセスのユースケースは以下の通りである。

- 組織の IdP の障害。
- 組織の IdP が関与するセキュリティインシデント。
- IAM Identity Center の障害。
- 組織のクラウドまたはIdPチーム全体が失われるような災害。これらのロールへのアクセスを監視し、ロールが環境へアクセスするために使用されたときにアラームとアラートをトリガーすることが重要である。

AWS Organizations Service Control Policiesは、組織の管理アカウントには適用されず、このアカウントへのアクセスは、管理アカウントとの信頼関係を考えると、組織全体に管理者ステータスを与えることになることを強調する価値がある。したがって、ブレークグラスIAMユーザーへのアクセスは、厳密に制御され、かつ事前に定義された厳格なプロセスを介してアクセスする必要があります。このプロセスでは、信頼できる個人がパスワードにアクセスし、別の信頼できる個人がハードウェアMFAキーにアクセスすることが多く、1セットのブレークグラス認証情報にアクセスするために通常2人が必要です。

あなたのユースケースが上記の4つのシナリオのいずれかでない場合、私たちはそれを真のブレークグラスシナリオとは見なさない。セキュリティインシデントが発生した場合、一般的にはAWSアカウント内の管理者ロールにアクセスできるユーザーが必要な修復を行うことを推奨します。このユーザーがタイムセンシティブなアクションを実行するために利用できないケースでは、高度に制限されたグループまたはグループのセットがIdP内で事前に構成され、それぞれが適切なAWSアカウントのセットへの適切なフェデレートアクセスを提供することをお勧めします。ユーザーは、優先度が高く一時的な変更要求を使用してこれらのグループのいずれかに追加されるか、または特権と信頼されたユーザーの選択されたグループがこれらのグループに事前に入力される可能性があります。

インシデントを調査するセキュリティチームは、このメカニズムを利用して、影響を受けるアカウントの読み取り専用ロールにアクセスするか、セキュリティツールアカウントを介して提供される読み取り専用アクセスメカニズムを使用することになる。要約すると、優先度の高い一般的な不定期アクセスシナリオを、標準的な連携アクセスプロセスおよび手順に組み込む必要がある。


注記
ブレークグラスへのアクセスには、組織管理アカウントの使用を推奨しますが、組織によっては、ブレークグラス専用のアカウントを追加することを選択する場合があります。この場合、組織管理アカウントに組織ブレークグラスユーザーが不要になるわけではありません。


# Recommended OUs and accounts
推奨OUの詳細と、セキュリティOUとアカウントの場合、推奨AWSアカウント一式を紹介します。

<img width="716" alt="image" src="https://user-images.githubusercontent.com/2577368/199180243-3a2464bb-dced-4e5c-a76c-14aceaa07932.png">

## Recommended OUs

お客様の要件によっては、推奨されるすべてのOUを設定する必要がない場合があります。AWSを採用し、ニーズについてより多くを学ぶことで、OUの全体的なセットを拡張することができます。AWSアカウントをどのように整理し始めるかの例については、AWSアカウントを整理するためのパターンを参照してください。

推奨されるOUは一般的なユースケースに沿ったものですが、ニーズを満たすために独自のOU構造を定義することもできます。このガイダンスは、ほとんどのお客様のニーズを満たすことを目的としています。ただし、すべてのユースケースに対応するものではありません。

推奨される OU の構成は以下の通りです。
...
セキュリティOUとインフラストラクチャーOUを基礎的なものとして分類しています。基礎OUは、アカウント、ワークロード、および他のAWSリソースを含み、AWS環境全体を保護しサポートするための共通のセキュリティとインフラストラクチャ機能を提供します。

基礎的なOUに存在するアカウント、ワークロード、およびデータは、通常、セキュリティ、インフラストラクチャ、および運用チームからのクロスファンクショナルな代表で構成される集中型クラウドプラットフォームまたはクラウドエンジニアリングチームによって所有されています。

お客様のアカウントの大部分は、他のOUに格納されています。これらのOUは、お客様のビジネス関連のワークロードを格納することを目的としています。また、ビジネス関連のサービスやデータのライフサイクル全体をサポートするツールやサービスも含まれています。
